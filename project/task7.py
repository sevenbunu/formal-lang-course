...
def matrix_based_cfpq(
    ...
    t = {nonterminal.value: lil_matrix((n, n), dtype=bool) for nonterminal in cfg_wnf.variables}
    reversed_productions = {}
    for production in cfg_wnf.productions:
        head, body = production.head.value, tuple(elem.value for elem in production.body)
        if body not in reversed_productions:
            reversed_productions[body] = [head]
        else:
            reversed_productions[body].append(head)
        for symbol, boolean_decomposition_for_symbol in automaton.boolean_decomposition.items():
    ...
    while not is_equal(prev_t, t):
        prev_t = {k: v.copy() for k, v in t.items()}
    ...
    for i in range(len(rows)):
        if rows[i] in automaton.start_states and cols[i] in automaton.final_states:
            result.add((automaton.int_to_state[rows[i]], automaton.int_to_state[cols[i]]))
    return result
